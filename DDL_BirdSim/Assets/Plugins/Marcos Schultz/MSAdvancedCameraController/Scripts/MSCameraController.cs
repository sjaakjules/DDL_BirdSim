...
public class MSACC_CameraType {
	...
	public enum TipoRotac{LookAtThePlayer, FirstPerson, FollowPlayer, Orbital, PanOrbital,Stop, StraightStop, OrbitalThatFollows, ETS_StyleCamera, FlyCamera_OnlyWindows}
...
public class MSACC_CameraSetting {
	...
	[Tooltip("In this class you can configure the 'Pan-Orbital' style cameras.")]
	public MSACC_SettingsCameraPanOrbital panOrbital;
...
[Serializable]
public class MSACC_SettingsCameraPanOrbital
{
	[Header("Settings")]
	[Range(0.01f, 2.0f)]
	...
	[Header("Custom Rotation Input")]
	[Tooltip("If this variable is true, the camera will only rotate when the key selected in the 'KeyToRotate' variable is pressed. If this variable is false, the camera can rotate freely, even without pressing any key.")]
	public bool rotateWhenClick = true;
	[Tooltip("Here you can select the button that must be pressed in order to rotate the camera.")]
	public string keyToRotate = "mouse 0";
	[Tooltip("Here you can select the button that must be pressed in order to pan the camera.")]
	public string keyToPan = "mouse 1";
...
public class MSCameraController : MonoBehaviour {
	...
	void Awake(){
		...
		for (int x = 0; x < cameras.Length; x++) {
			if (cameras [x]._camera) {
				...
				if (cameras[x].rotationType == MSACC_CameraType.TipoRotac.PanOrbital)
				{
					cameras[x]._camera.transform.parent = temp.transform;
					cameras[x]._camera.transform.LookAt(target);
					xOrbit[x] = cameras[x]._camera.transform.eulerAngles.y;
					...
					distanceFromOrbitalCamera[x] = Vector3.Distance(cameras[x]._camera.transform.position, targetTransform.position);
					distanceFromOrbitalCamera[x] = Mathf.Clamp(distanceFromOrbitalCamera[x], CameraSettings.panOrbital.minDistance, CameraSettings.panOrbital.maxDistance);
				}
	...
	void ManageCameras(){
		if (changeCam) {
			...
			for (int x = 0; x < cameras.Length; x++) {
				...
				if (cameras [x].rotationType == MSACC_CameraType.TipoRotac.Orbital || cameras [x].rotationType == MSACC_CameraType.TipoRotac.OrbitalThatFollows|| 
					cameras[x].rotationType == MSACC_CameraType.TipoRotac.PanOrbital) {
					cameras [x]._camera.transform.LookAt (target);
					xOrbit [x] = cameras [x]._camera.transform.eulerAngles.y;
					yOrbit [x] = cameras [x]._camera.transform.eulerAngles.x;
					distanceFromOrbitalCamera [x] = Vector3.Distance (cameras [x]._camera.transform.position, targetTransform.position);
			...
			if (cameras [index].rotationType == MSACC_CameraType.TipoRotac.FirstPerson || cameras [index].rotationType == MSACC_CameraType.TipoRotac.ETS_StyleCamera ||
				cameras [index].rotationType == MSACC_CameraType.TipoRotac.Orbital || cameras[index].rotationType == MSACC_CameraType.TipoRotac.PanOrbital || 
				cameras [index].rotationType == MSACC_CameraType.TipoRotac.OrbitalThatFollows) {
		...
		case MSACC_CameraType.TipoRotac.Orbital:
			...
			case MSACC_CameraType.TipoRotac.PanOrbital:
				//raycast hit
				RaycastHit hitCamPanOrbital;
				float minDistanc = CameraSettings.panOrbital.minDistance;
				if (Physics.Linecast(targetTransform.position, cameras[index]._camera.transform.position, out hitCamPanOrbital))
				{
					if (!CameraSettings.panOrbital.ignoreCollision)
					{
						distanceFromOrbitalCamera[index] = Vector3.Distance(targetTransform.position, hitCamPanOrbital.point);
						minDistanc = Mathf.Clamp(distanceFromOrbitalCamera[index], minDistanc * 0.5f, CameraSettings.panOrbital.maxDistance);
					}
				}
				//getInputs
				float xInputPanOrb = _horizontalInputMSACC;
				float yInputPanOrb = _verticalInputMSACC;
				if (CameraSettings.panOrbital.invertXInput)
				{
					xInputPanOrb = -_horizontalInputMSACC;
				}
				if (CameraSettings.panOrbital.invertYInput)
				{
					yInputPanOrb = -_verticalInputMSACC;
				}
				if (CameraSettings.panOrbital.rotateWhenClick)
				{
					if (Input.GetKey(CameraSettings.panOrbital.keyToRotate) || _enableMobileInputs)
					{
						xOrbit[index] += xInputPanOrb * (CameraSettings.panOrbital.sensibility * distanceFromOrbitalCamera[index]) / (distanceFromOrbitalCamera[index] * 0.5f);
						yOrbit[index] -= yInputPanOrb * CameraSettings.panOrbital.sensibility * (CameraSettings.panOrbital.speedYAxis * 10.0f);
					}
					if (Input.GetKey(CameraSettings.panOrbital.keyToPan))
					{
						//targetTransform.Translate(0.0f,yInputPanOrb * CameraSettings.panOrbital.sensibility,0.0f,Space.World);
						Vector3 vec = (cameras[index]._camera.transform.TransformDirection(Vector3.right));
						targetTransform.Translate(vec.x* -xInputPanOrb * CameraSettings.panOrbital.sensibility,
													-yInputPanOrb * CameraSettings.panOrbital.sensibility,
													vec.z* -xInputPanOrb * CameraSettings.panOrbital.sensibility, Space.World);
					}
				}
				else
				{
				...
				//move - rotation
				yOrbit[index] = ClampAngle(yOrbit[index], CameraSettings.panOrbital.minAngleY, CameraSettings.panOrbital.maxAngleY);
				Quaternion quaternToEuler = Quaternion.Euler(yOrbit[index], xOrbit[index], 0);
				distanceFromOrbitalCamera[index] = Mathf.Clamp(distanceFromOrbitalCamera[index] - _scrollInputMSACC * (CameraSettings.panOrbital.speedScrool * 50.0f), minDistanc, CameraSettings.panOrbital.maxDistance);
				Vector3 zPos = new Vector3(0.0f, 0.0f, -distanceFromOrbitalCamera[index]);
				Vector3 nextPosCamera = quaternToEuler * zPos + targetTransform.position;
				Vector3 currentPosCamera = cameras[index]._camera.transform.position;
				Quaternion camRota = cameras[index]._camera.transform.rotation;
				cameras[index]._camera.transform.rotation = Quaternion.Lerp(camRota, quaternToEuler, Time.deltaTime * 5.0f * timeScaleSpeed);
				cameras[index]._camera.transform.position = Vector3.Lerp(currentPosCamera, nextPosCamera, Time.deltaTime * 5.0f * timeScaleSpeed);
				break;
