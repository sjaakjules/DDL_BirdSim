...
namespace DeepDesignLab.PointCloud {
    public class PointOctreeNode<T> {
        ...
        protected Bounds bounds = default(Bounds);

        // Objects in this node
        readonly protected List<OctreeObject> objects = new List<OctreeObject>();
        protected T averageObject;

        // Child nodes, if any
        protected PointOctreeNode<T>[] children = null;

        protected bool HasChildren { get { return children != null; } }
        ...
        public void SetChildren(PointOctreeNode<T>[] childOctrees) {
            if (childOctrees.Length != 8) {
                UnityEngine.Debug.LogError("Child octree array must be length 8. Was length: " + childOctrees.Length);
                return;
            }
        ...
        void SubAdd(T obj, Vector3 objPos) {
            ...
            if (!HasChildren) {
                ...
                if (children == null) {
                    Split();
                    if (children == null) {
                        UnityEngine.Debug.LogError("Child creation failed for an unknown reason. Early exit.");
                        return;
                    }
