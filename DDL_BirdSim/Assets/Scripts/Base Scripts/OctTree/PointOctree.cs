    ...
    public class PointOctree<T>  {
        ...
        public PointOctree(float initialWorldSize, Vector3 initialWorldPos, float minNodeSize) {
            if (minNodeSize > initialWorldSize) {
                UnityEngine.Debug.LogWarning("Minimum node size must be at least as big as the initial world size. Was: " + minNodeSize + " Adjusted to: " + initialWorldSize);
        ...
        public void Add(T obj, Vector3 objPos) {
            ...
            while (!rootNode.Add(obj, objPos)) {
                ...
                if (++count > 20) {
                    UnityEngine.Debug.LogError("Aborted Add operation as it seemed to be going on forever (" + (count - 1) + ") attempts at growing the octree.");
                    return;
                }
            }
            Count++;
